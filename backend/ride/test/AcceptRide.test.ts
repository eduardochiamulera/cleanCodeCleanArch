...
test("Não deve aceitar uma corrida duas vezes", async function () {
    const inputSignup = {
        name: "John Doe",
        email: `john.doe${Math.random()}@gmail.com`,
        cpf: "97456321558",
    ...
    expect(async () => await acceptRide.execute(inputAcceptRide)).rejects.toThrow(new Error("Ride already accepted"));
});
